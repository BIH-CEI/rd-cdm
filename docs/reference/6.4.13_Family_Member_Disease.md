---
search:
  boost: 5.0
---

# Slot: 6.4.13 Family Member Disease (6.4.13 Family Member Disease) 


_Indicates whether the selected family member is affected by the same rare disease as the individual._



<div data-search-exclude markdown="1">



URI: [LOINC:75315-2](http://loinc.org75315-2)
Alias: 6.4.13_Family_Member_Disease

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [FamilyHistory](FamilyHistory.md) | 6 |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [xsd:string](http://www.w3.org/2001/XMLSchema#string) |
| Domain Of | [FamilyHistory](FamilyHistory.md) |
| Slot URI | [LOINC:75315-2](http://loinc.org75315-2) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information



### Annotations

| property | value |
| --- | --- |
| ordinal | 6.4.13 |
| section | 6.4 Family History |
| data_type | Code |
| data_specification | Ontology Class, ORDO, ICD-10-CM, ICD-11, MONDO, OMIM_p |
| fhir_expression | FamilyMemberHistory.condition.code |
| fhir_datatype | Family.relatives |
| phenopacket_element | Family.relatives |
| phenopacket_datatype | Family.relatives |




### Schema Source


* from schema: https://github.com/BIH-CEI/rd-cdm/linkml/rd_cdm_profile.yaml




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | LOINC:75315-2 |
| native | rdcdm:6.4.13_Family_Member_Disease |




## LinkML Source

<details>
```yaml
name: 6.4.13 Family Member Disease
annotations:
  ordinal:
    tag: ordinal
    value: 6.4.13
  section:
    tag: section
    value: 6.4 Family History
  data_type:
    tag: data_type
    value: Code
  data_specification:
    tag: data_specification
    value: Ontology Class, ORDO, ICD-10-CM, ICD-11, MONDO, OMIM_p
  fhir_expression:
    tag: fhir_expression
    value: FamilyMemberHistory.condition.code
  fhir_datatype:
    tag: fhir_datatype
    value: Family.relatives
  phenopacket_element:
    tag: phenopacket_element
    value: Family.relatives
  phenopacket_datatype:
    tag: phenopacket_datatype
    value: Family.relatives
description: Indicates whether the selected family member is affected by the same
  rare disease as the individual.
title: 6.4.13 Family Member Disease
from_schema: https://github.com/BIH-CEI/rd-cdm/linkml/rd_cdm_profile.yaml
rank: 1000
slot_uri: LOINC:75315-2
alias: 6.4.13_Family_Member_Disease
domain_of:
- FamilyHistory
range: string

```
</details></div>