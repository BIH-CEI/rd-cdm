# AUTO-GENERATED by src/rd_cdm/utils/gen_pydantic.py (PythonGenerator)
...
class RdCdm(YAMLRoot):
    ...
    rd_cdm_version: str = None
    rd_cdm_date: Union[str, XSDDate] = None
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        if self._is_empty(self.rd_cdm_version):
            self.MissingRequiredField("rd_cdm_version")
        if not isinstance(self.rd_cdm_version, str):
            self.rd_cdm_version = str(self.rd_cdm_version)

        if self._is_empty(self.rd_cdm_date):
            self.MissingRequiredField("rd_cdm_date")
        if not isinstance(self.rd_cdm_date, XSDDate):
            self.rd_cdm_date = XSDDate(self.rd_cdm_date)

        self._normalize_inlined_as_list(slot_name="code_systems", slot_type=CodeSystem, key_name="id", keyed=True)

        self._normalize_inlined_as_list(slot_name="data_elements", slot_type=DataElement, key_name="ordinal", keyed=False)

        self._normalize_inlined_as_list(slot_name="value_sets", slot_type=ValueSet, key_name="id", keyed=False)
...
class ValueSet(YAMLRoot):
    ...
    def __post_init__(self, *_: str, **kwargs: Any):
        ...
        self._normalize_inlined_as_list(slot_name="codes", slot_type=Coding, key_name="system", keyed=False)
...
slots.rdCdm__rd_cdm_version = Slot(uri=DEFAULT_.rd_cdm_version, name="rdCdm__rd_cdm_version", curie=DEFAULT_.curie('rd_cdm_version'),
                   model_uri=DEFAULT_.rdCdm__rd_cdm_version, domain=None, range=str)

slots.rdCdm__rd_cdm_date = Slot(uri=DEFAULT_.rd_cdm_date, name="rdCdm__rd_cdm_date", curie=DEFAULT_.curie('rd_cdm_date'),
                   model_uri=DEFAULT_.rdCdm__rd_cdm_date, domain=None, range=Union[str, XSDDate])
